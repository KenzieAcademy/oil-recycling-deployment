...
docker run -e NODE_ENV=production -e REACT_APP_OIL_RECYCLING_API=$BACKEND_API --name oil-recycling-fe -p 80:3000 -d kenzieacademy/oil-recycling-project-fe:$TAG /bin/bash -c "npm run build && npm install -g serve@7.2.0 && serve -s build"
